...
function addFooter() {
    ...
    <div class="copy-right">
        <p><a href="index.html"> Power accessories </a> - All rights reserved © 2021 - Designed by
            <span style="color: #eee; font-weight: bold">ThanhtungShop</span></p>
    </div>`);
}
...
// Hàm chuẩn hóa tên sản phẩm, loại bỏ dấu
...
function isProductExist(productName) {
    const productList = ["laptop", "phim", "chuot"]; // Danh sách sản phẩm
    return productList.includes(productName.toLowerCase());
}

// Phân tích URL và hiển thị sản phẩm
function phanTich_URL_chiTietSanPham() {
    const url = window.location.href;
    const params = new URL(url).searchParams;
    const nameProduct = params.get('name'); // Lấy tên sản phẩm từ URL
    const normalizedProductName = removeDiacritics(nameProduct); // Chuẩn hóa tên sản phẩm

    // Kiểm tra sản phẩm có tồn tại không
    if (!isProductExist(normalizedProductName)) {
        document.getElementById("productNotFound").style.display = "block"; // Hiển thị thông báo không tìm thấy
    } else {
        // Hiển thị trang chi tiết sản phẩm ở đây (có thể thêm logic để hiển thị thông tin sản phẩm)
        document.getElementById("productNotFound").style.display = "none"; // Ẩn thông báo
        // Thêm mã hiển thị sản phẩm ở đây
    }
}

// Gọi hàm khi trang được tải
window.onload = phanTich_URL_chiTietSanPham;
